...
public class scalogram extends Plot implements JSci.awt.ColorScheme {
    double [][] XY;
  ...
  public Color getColor(float f) {
	...
	public void plot(AbstractDrawer draw, Color c) {
		...
		aDraw.setColor(new Color(1f-c1,1f-c2,1f-c3));
		aDraw.drawRect(Contourx+ColorScaleWidth,Contoury-1,xCoordMax,yCoordMax);
 		aDraw.drawRect(Contourx-1,Contoury-1,ColorScaleWidth,yCoordMax);
                ...
                double step=yCoordMax/ColorResolution;
                for(float y=0;y<yCoordMax;y+=step) {
    ...
    for(int k=0;k<XY.length;k++) {
			for(int l=0;l<XY[k].length;l++) {
                ...
                double  prev = (int)Math.floor(l*deltax[k])+2*Contourx+ColorScaleWidth;
                int prev1 = (int)prev;
                int prev2 = (int)(prev+deltax[k]);
                aDraw.fillRect(prev1,  k*deltay+Contoury,  prev2, (k+1)*deltay+Contoury);
			}
    
                 }
      aDraw.canvas.repaint();
             }
...
private static double[][] createContourData() {
                int dataLenRows=90;
                int dataLenCols = 120;
                double data[][]=new double[dataLenRows][dataLenCols];
                double x,y;
                for(int i=0;i<dataLenRows;i++) {
                        for(int j=0;j<dataLenCols;j++) {
                                x=(i-dataLenRows/2.0)/dataLenRows;
                                y=(j-dataLenCols/2.0)/dataLenCols;
	...
	public static void main(String[] args) {
		Plot2DPanel p2 = new Plot2DPanel();
                ...
                p2.addScalogramPlot("Contour test", data);
