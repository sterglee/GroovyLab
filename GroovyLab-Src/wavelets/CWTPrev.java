...
public class CWTPrev implements Serializable{
	private double[][] Re;
	private double[][] Im;
	public double[] f;
	public double[] t;
	public double[] deltaStept;
	public CWTPrev(double[] y){
		   this(y,y.length,(double) y.length/4,100,"log",8,3);
		}	
	public CWTPrev(double[] y,int fs){
		   this(y,fs,(double) fs/4,100,"log",8,3);
		}	
	public CWTPrev(double[] y,int fs,double fmax){
	   this(y,fs,fmax,100,"log",16,3);
	}
	public CWTPrev(double[] y,int fs,double fmax, int maxNf){
		   this(y,fs,fmax,maxNf,"log",16,3);
		}
	public CWTPrev(double[] y,int fs,double fmax, int maxNf, int stepfac){
		   this(y,fs,fmax,maxNf,"log",stepfac,3);
		}
	public CWTPrev(double[] y,int fs,double fmax, int maxNf, int stepfac, int df0){
		   this(y,fs,fmax,maxNf,"log",stepfac,df0);
		}
        
        
        public CWTPrev(Matrix  y,int fs){
		   this(y,fs,(double) fs/4,100,"log",8,3);
		}	
	public CWTPrev(Matrix y,int fs,double fmax){
	   this(y,fs,fmax,100,"log",16,3);
	}
	public CWTPrev(Matrix  y,int fs,double fmax, int maxNf){
		   this(y,fs,fmax,maxNf,"log",16,3);
		}
	public CWTPrev(Matrix  y,int fs,double fmax, int maxNf, int stepfac){
		   this(y,fs,fmax,maxNf,"log",stepfac,3);
		}
	public CWTPrev(Matrix y, int fs,double fmax, int maxNf, int stepfac, int df0){
		   this(y,fs,fmax,maxNf,"log",stepfac,df0);
		}
        
        
        
	public CWTPrev(double[] y,int fs,double fmax, int maxNf, String linlog, int stepfac, int df0){
...
//		            		System.out.println("morlet.getRe(startw+i): " + morlet.getRe(startw+i));
        ...
        public CWTPrev(Matrix ym,int fs,double fmax, int maxNf, String linlog, int stepfac, int df0){
            double [] y = ym.getArray()[0];
		// Calculation of the transform
		// used to calc between a fictiv T=1s and the real
		double ffac = (double) fs/y.length; 
		// maximum number of dyades: 2^6<n<2^7 -> maxNd = 6
...
//		            		System.out.println("morlet.getRe(startw+i): " + morlet.getRe(startw+i));
	...
	public double[][] getAbs(){
        ...
        // help methods 
	// returning the absolute value
	public groovySci.math.array.Matrix getMAbs(){
		double out[][] = new double[Re.length][];
		...
		return new groovySci.math.array.Matrix(out);
	}
