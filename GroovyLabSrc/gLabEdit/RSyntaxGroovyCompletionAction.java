...
static public String methodSubString="";
    ...
    public void performCompletion() {
      ...
      //   form total word that is under caret position
       ...
       StringBuilder sb = new StringBuilder(wb.toString());
       StringBuilder rsb = new StringBuilder(wb.toString());
       int p = 0;
       for (int k= wb.length()-1; k>=0; k--)
           rsb.setCharAt(p++, sb.charAt(k));
        
       // concatenate to form the whole word
       String  wordAtCursor = rsb.toString()+wa.toString();
...
public static void collectInfoWithReflection(String nameOfType)
{
    ...
    Package [] pkg = Package.getPackages(); // get all the packages currently known for the caller's ClassLoader instance
...
for (int k=0; k<classMethods.length; k++) {   // all class methods
      ...
      String nameAndParams = methodName+"(" + args.toString()+")";
      
      scanMethods.add(nameAndParams);
