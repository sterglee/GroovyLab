...
public class Scs_ereach {
     ...
     * @param w
     *            size n, work array, w[0.f.n-1]>=0 on input, unchanged on output
     * 
     * @return top in successful, -1 on error
     */
    public static int cs_ereach(Scs A, int k, int[] parent, int[] s, int s_offset, int[] w) {
        int i, p, n, len, top, Ap[], Ai[];
        if (!Scs_util.CS_CSC(A) || parent == null || s == null || w == null)
            return (-1); /* check inputs */
        top = n = A.n;
        Ap = A.p;
        Ai = A.i;
        Scs_util.CS_MARK(w, k); /* mark node k as visited */
        for (p = Ap[k]; p < Ap[k + 1]; p++) {
            i = Ai[p]; /* A(i,k) is nonzero */
            if (i > k)
                continue; /* only use upper triangular part of A */
            for (len = 0; !Scs_util.CS_MARKES(w, i); i = parent[i]) /* traverse up etree*/
            {
                s[s_offset + len++] = i; /* L(k,i) is nonzero */
                Scs_util.CS_MARK(w, i); /* mark i as visited */
            }
            while (len > 0)
                s[s_offset + --top] = s[s_offset + --len]; /* push path onto stack */
        }
        for (p = top; p < n; p++)
            Scs_util.CS_MARK(w, s[s_offset + p]); /* unmark all nodes */
        Scs_util.CS_MARK(w, k); /* unmark node k */
