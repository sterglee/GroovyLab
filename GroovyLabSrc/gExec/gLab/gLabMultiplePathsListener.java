...
public class gLabMultiplePathsListener  implements TreeSelectionListener   {
    JTree  pathsTree;
  // the selectedValue keeps the full pathname of the selected object for further processing
    ...
    public gLabMultiplePathsListener(JTree classPathsTree) {
    ...
    public void explicitUpdate()  {
        ...
        // for nested objects, their path is repeated as the parent node, so concatenate the parent and the filename to build the complete path
          gLabMultiplePathsListener.selectedValues[k] = objPath[len-1].toString();
          gLabMultiplePathsListener.selectedPaths[k] = selectedValues[k].substring(0, selectedValues[k].lastIndexOf(File.separator));
      }
