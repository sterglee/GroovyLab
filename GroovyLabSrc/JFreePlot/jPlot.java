...
/* public void load(String fileName) throws IOException {
    ...
    private int getLineIndex(String lineId) {
        ...
        public String[] jplot(Matrix  x, Matrix  y, int dummy, Matrix ... args) {
        ...
        int noOfSeries = args.length / 2 + 1; 
        String [] seriesKeys = new String[noOfSeries];
        seriesKeys[0] = jplot(x.getv(), y.getv(), "y", "")[0];
       ...
       public String[] jplot(Matrix x, Matrix y, double[]... args) {
        return jplot(x.getv(), y.getv(), args);
    }

       public String[] jplot(double [] x, Matrix y, double[]... args) {
        return jplot(x, y.getv(), args);
    }

      public String[] jplot(Matrix  x, double [] y, double[]... args) {
        return jplot(x.getv(), y, args);
    }

        public String[] jplot(Matrix  x) {
            // construct implicitly the axis
           double [] vx = x.getv();
            int sigLen = vx.length;
           double [] taxis = new double[sigLen];
           for (int k=0; k<sigLen; k++)
               taxis[k] = k;
         return jplot(taxis, vx);
    }
        
            
        public String[] jplot(Matrix x, String lineSpec, Object... args) {
    // construct implicitly the axis
           double [] vx = x.getv();
            int sigLen = vx.length;
           double [] taxis = new double[sigLen];
           for (int k=0; k<sigLen; k++)
               taxis[k] = k;
         return jplot(taxis, vx, lineSpec, args);
        }
        
           public String[] jplot(double []x, Matrix  y, String lineSpec, String legend, Object... args) {
          return   jplot(x, y.getv(), lineSpec, legend, args);
    }
    
       public String[] jplot(Matrix x, Matrix  y, String lineSpec, String legend, Object... args) {
           return   jplot(x.getv(), y.getv(), lineSpec, legend, args);
    }
    
       public String[] jplot(Matrix x, double []  y, String lineSpec, String legend, Object... args) {
           return  jplot(x.getv(), y, lineSpec, legend, args);
    }
       
          public String[] jplot(Matrix x, String lineSpec, String legend,  Object... args) {
    // construct implicitly the axis
           double [] vx = x.getv();
            int sigLen = vx.length;
           double [] taxis = new double[sigLen];
           for (int k=0; k<sigLen; k++)
               taxis[k] = k;
         return jplot(taxis, vx, lineSpec, legend, args);
        }
        
          
        

public String jplot(Matrix  x, Paint color, Shape marker,  float[] style,  String legend) {
// construct implicitly the axis
           double [] vx = x.getv();
            int sigLen = vx.length;
           double [] taxis = new double[sigLen];
           ...
           return jplot(taxis, vx, color, marker, style, legend);
}


public String addPlot(Matrix x, Matrix  y, String lineSpec) {
        // dummy parameter is needed for disambiguation of overloaded definition
        // Matrix x, Matrix y, Matrix..
        return addPlot(x.getv(), y.getv(), lineSpec, "");
    }

    public String addPlot(Matrix x, Matrix  y, int dummy) {
        return addPlot(x.getv(), y.getv(), "");
    }
    
    
    
public  String addPlot(Matrix x, Matrix y, String lineSpec, String legend) {
