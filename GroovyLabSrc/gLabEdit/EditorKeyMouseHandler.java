  ...
  public  RSyntaxDocument updateDocument()  {
   ...
   try {
    ...
    break;
     ...
     if (GlobalValues.globalJavaEvaluator==null)                
         GlobalValues.globalJavaEvaluator = new Evaluator();
     
      currentText = getSelectedTextOrCurrentLine();
      Either<Throwable, Evaluation> result;
     
      result =  GlobalValues.globalJavaEvaluator.evaluate(currentText);
       
      GlobalValues.consoleOutputWindow.append(result.toString());
        Sequence<Result> allResults = GlobalValues.globalJavaEvaluator.context().results();
        if (allResults.isEmpty()==false) {
        allResults.forEach((Result x) -> { 
