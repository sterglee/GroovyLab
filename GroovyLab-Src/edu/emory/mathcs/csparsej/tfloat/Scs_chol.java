...
public class Scs_chol {
    ...
    public static Scsn cs_chol(Scs A, Scss S) {
        ...
        pinv = S.pinv;
        parent = S.parent;
        C = pinv != null ? Scs_symperm.cs_symperm(A, pinv, true) : A;
        s = c;
        int s_offset = n;
        Cp = C.p;
        Ci = C.i;
        Cx = C.x;
        N.L = L = Scs_util.cs_spalloc(n, n, cp[n], true, false); /* allocate result */
        Lp = L.p;
        Li = L.i;
        Lx = L.x;
        for (k = 0; k < n; k++)
        ...
        for (k = 0; k < n; k++) /* compute L(k,:) for L*L' = C */
        {
            ...
            top = Scs_ereach.cs_ereach(C, k, parent, s, s_offset, c); /* find pattern of L(k,:) */
