...
public class JavaCompile  extends  ClassLoader  {
 ...
 public  boolean compileFile( String sourceFile) { 
    ...
    sourceFileList.add(new File(sourceFile));   // add the source file to compile
    ...
    if (compileResult == false) {
        ...
        compResultsArea.setFont(new Font("Times New Roman", Font.BOLD, 16));
        ...
        if (response == JOptionPane.YES_OPTION)   {   // edit it with jLabEditor
            GlobalValues.myGEdit = new  gLabEditor(sourceFile);
 ...
 public  Class  loadNewClass(String javaFile, String packageName)  {
	...
	if (bytes_read != size)   throw new IOException("cannot read entry");
       ...
       try {
        ...
        if (myClass!=null)
            System.out.println("class: "+packageAppendedFileName+" defined");
        else
            System.out.println("class: "+packageAppendedFileName+" null");
