...
import groovySci.math.plot.Plot3DPanel;
...
public class Axis implements Plotable, BaseDependant, Editable {
	 ...
	 */
        ...
        protected int index;
        protected Base base;
        boolean visible = true; // Visibility of the whole axis
        protected Color color;  // Color in which the name of the axis is displayed.
        protected String label;  // axis label
        boolean gridVisible = true;  // visibility of the grid
    ...
    @Override
    public void setVisible(boolean v) { //  Sets the visibility of the whole axis object.
	visible = v;
}

    @Override
    public boolean getVisible() { // Returns the visibility of the whole axis object.
	return visible;
}
    ...
    public void setGridVisible(boolean v) {  // sets the visibility of the light lines and their labels
	gridVisible = v;
}

    @Override
    public void setColor(Color c) {  // sets the color used to display the axis label
	color = c;
	darkLabel.setColor(color);
}

    @Override
    public Color getColor() {   // returns the color of the axis label
	return color;
}

    public void setLegend(String _label) {  // sets the label of the axis
	label = _label;
	darkLabel.setText(label);
}

     public String getLegend() {  // returns the label of the axis
	return label;
}
...
public void setLightLabels() {
    ...
    for (int i = 0; i < lightLabels.length; i++) {
        ...
        if (base.getAxeScale(index).startsWith(Base.LINEAR) || base.getAxeScale(index).startsWith(Base.STRINGS)) {
