...
public class CWT implements Serializable{
    ...
    public CWT(Matrix  y, int fs){
       this(y, fs, (double) fs/4,100,"log",8,3);
    }	

    public CWT(Matrix y, int fs, double fmax){
        this(y,fs,fmax,100,"log",16,3);
    }

    public CWT(Matrix  y,int fs,double fmax, int maxNf){
        this(y,fs,fmax,maxNf,"log",16,3);
    }
	public CWT(Matrix  y,int fs,double fmax, int maxNf, int stepfac){
		   this(y,fs,fmax,maxNf,"log",stepfac,3);
		}
	public CWT(Matrix y, int fs,double fmax, int maxNf, int stepfac, int df0){
...
//		            		System.out.println("morlet.getRe(startw+i): " + morlet.getRe(startw+i));
        ...
        public CWT(Matrix ym, int fs, double fmax,  int maxNf, String linlog, int stepfac, int df0){
            
            double [][] ymv = ym.getArray();
            int rows = ymv.length;
            int cols = ymv[0].length;
            if (cols < rows)   //  a row vector should be passed, if not transform it
                ym = groovySci.math.array.Matrix.transpose(ym);
