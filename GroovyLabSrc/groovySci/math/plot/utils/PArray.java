package groovySci.math.plot.utils;
...
public class PArray {
    ...
    // create a new hash map that has reversed keys and values (i.e. has as keys the values and as values the keys of the original hash map)
    ...
    return reverseMap;
     ...
     // create and return a hash map of Strings from  the list of Strings
    private static HashMap<String, Double> mapStringArray(double minvalue, double step, List<String> array) {
        ...
        // for all Strings in the rray of strings, insert them in the map
  ...
  public static String cat(Object[][] array) {
   ...
   public static String[] duplicate(int m, String c) {
    ...
    public static boolean isLength(double[] M, int n) {
        ...
        // converts the elemenents of a  Vector with double [] elements to a double[][]
    static public double [] toArr(Vector <Double> x) {
            int len = x.size();
           double []  elems = new double[len];
           for (int k=0; k<len; k++)
               elems[k] = (double) x.elementAt(k);
           return elems;
    }

        // converts the elemenents of a  Vector with double [] elements to a double[][]
        // the zero'th element of the vector is redundant 
      static public double [][] toArrv(Vector <double[] > x) {
           double[] elem = (double []) x.elementAt(0);  // each element of the Vector is a double[] array
           
           int npoints= x.size();  // number of elements of the Vector
           int vecDim = elem.length;   // dimension of each Vector element
    
             double [][] values = new double[vecDim][npoints];
    for (int xv=0; xv<npoints; xv++)    {
        elem = (double []) x.elementAt(xv);  // current Vector element
        for (int yv=0; yv<vecDim-1; yv++)
            values[yv][xv] =elem[yv+1];
            }
              return values;
           }
