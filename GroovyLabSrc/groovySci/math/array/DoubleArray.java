...
import groovySci.math.array.util.Function;
import groovySci.math.array.util.IndexFunction;
import groovySci.math.array.util.Random;
import groovySci.math.array.util.Sorting;
import gExec.Interpreter.GlobalValues;

// provides a lot of manipulations for double [][] arrays as static methods
...
// adds v2 to all the columns of the receiver
public static double [][] Add(double [][]receiver,  double [] v2) {
     int nrows = receiver.length;
     int ncols = receiver[0].length;
     double [][]res = new double[nrows][ncols];
          for (int row=0; row<nrows; row++)
            for (int col=0; col<ncols; col++)
               res[row][col] = receiver[row][col]+v2[row];
        
        return res;
 }

// subtracts v2 from all the columns of the receiver
public static double [][] Subtract(double [][]receiver,  double [] v2) {
   int nrows = receiver.length;
   int ncols = receiver[0].length;
   double [][]res = new double[nrows][ncols];
   for (int row=0; row<nrows; row++)
...
// multiply rows of matrix M with the vector m
public static double [] Multiply(double [][]M, double [] m) {
    int Nrows = M.length;
...
return Ms;
  ...
  public static double[][]  log2(double [][] M) {
                ...
                for (int i = 0; i < M.length; i++)
		    for (int j = 0; j < M[i].length; j++)
				Ms[i][j] = Math.log(M[i][j])/GlobalValues.log2Conv;
		return Ms;
	}
  ...
  public static double[][]  toDegrees(double [][] M ) {
                double [][] Ms = new double[M.length][M[0].length];
                for (int i = 0; i < M.length; i++)
		    for (int j = 0; j < M[i].length; j++)
 ...
 // add now for double [] type, some operations
  public static double  [] sin(double [] M) {
    double [] Ms = new double[M.length];
    for (int i = 0; i < M.length; i++)
            Ms[i] = Math.sin(M[i]);
        return Ms;
    }
...
public static double  [] cosh(double [] M) {
  ...
  public static double  [] log2(double [] M) {
    double [] Ms = new double[M.length];
    for (int i = 0; i < M.length; i++)
            Ms[i] = Math.log(M[i])/GlobalValues.log2Conv;
        return Ms;
    }
  ...
  public static double  [] log(double [] M, double exponent) {
    double [] Ms = new double[M.length];
    for (int i = 0; i < M.length; i++)
            Ms[i] = Math.pow(M[i], exponent);
        return Ms;
    }
...
public static double[] one(int m) {
    ...
    and columns j1 through j2 inclusive. Example:
...
public static String toString(double [][]d) {
    return groovySci.math.array.Matrix.printArray(d);
}
        // print methods
...
public static String ToString(double[]... v) {
 ...
 for (int i = 0; i < v.length; i++) {  // all rows
   if (i >= maxRows) {
    str.append(".....................................\n");
     break;
     }
for (int j = 0; j < v[i].length; j++) { // all columns
  if (j >= maxCols) {
   str.append("..... ");
    break;
     }
...
return str.toString();
	 ...
	 * Must import groovySci.math.array.util.*; to get Function interface.
