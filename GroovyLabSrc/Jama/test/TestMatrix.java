...
public class TestMatrix {
   public static void main (String argv[]) {
      jMatrix A,B,C,Z,O,I,R,S,X,SUB,M,T,SQ,DEF,SOL;
...
**/
      ...
      R = jMatrix.random(A.getRowDimension(),A.getColumnDimension());
      A = R;
      try {
        S = A.minus(S);
      ...
      A = R.copy();
      A.minusEquals(R);
      Z = new jMatrix(A.getRowDimension(),A.getColumnDimension());
      try {
        A.minusEquals(S);
      ...
      A = R.copy();
      B = jMatrix.random(A.getRowDimension(),A.getColumnDimension());
      C = A.minus(B); 
      try {
        S = A.plus(S);
      ...
      A = R.copy();
      O = new jMatrix(A.getRowDimension(),A.getColumnDimension(),1.0);
...
**/
      print("\nTesting I/O methods...\n");
	    ...
	    fmt.setDecimalFormatSymbols(new DecimalFormatSymbols(Locale.US));
            ...
            A.print(FILE,fmt,10);
            FILE.close();
            R = jMatrix.read(new BufferedReader(new FileReader("JamaTestMatrix.out")));
...
**/
      ...
      X = A.inverse();
      try {
         check(A.times(X),jMatrix.identity(3,3));
         try_success("inverse()...","");
      } catch ( java.lang.RuntimeException e ) {
         errorCount = try_failure(errorCount,"inverse()...","incorrect inverse calculation");
      }
      O = new jMatrix(SUB.getRowDimension(),1,1.0);
      ...
      X = Chol.solve(jMatrix.identity(3,3));
      try {
         check(A.times(X),jMatrix.identity(3,3));
   ...
   private static void check(jMatrix X, jMatrix Y) {
      double eps = Math.pow(2.0,-52.0);
