...
if (exists) {
 ...
 compileMenu.setFont(GlobalValues.uifont);
        ...
        JMenuItem executeJavaREPL = new JMenuItem("Execute with Java REPL (F7)");
        executeJavaREPL.addActionListener( (ActionEvent e)-> {
        JOptionPane.showMessageDialog(null, "Use F7 to evaluate Java code with Java REPL", "Java evaluation", JOptionPane.INFORMATION_MESSAGE);
         }
        );        
        executeJavaREPL.setFont(GlobalValues.uifont);
        compileMenu.add(executeJavaREPL);
 
        JMenuItem resetJavaREPL = new JMenuItem("Reset Java REPL");
        resetJavaREPL.addActionListener( (ActionEvent e)-> { gExec.Interpreter.GlobalValues.globalJavaEvaluator.reset(); });
        resetJavaREPL.setFont(GlobalValues.uifont);
        compileMenu.add(resetJavaREPL);
        
        
        JMenuItem resultsJavaREPL = new JMenuItem("Results of Java REPL");
        resultsJavaREPL.addActionListener( (ActionEvent e)-> { 
            System.out.println(gExec.Interpreter.GlobalValues.globalJavaEvaluator.results().toString()); });
        resultsJavaREPL.setFont(GlobalValues.uifont);
        compileMenu.add(resultsJavaREPL);
        
        JMenuItem expressionsJavaREPL = new JMenuItem("Expressions Java REPL");
        expressionsJavaREPL.addActionListener( (ActionEvent e)-> 
          { System.out.println(gExec.Interpreter.GlobalValues.globalJavaEvaluator.expressions().toString()); });
        expressionsJavaREPL.setFont(GlobalValues.uifont);
        compileMenu.add(expressionsJavaREPL);
    ...
    insertMapComputeFunction.addActionListener(new ActionListener() {
            ...
            public void actionPerformed(ActionEvent e) {
             ...
             String functionStaticText = "\n@groovy.transform.CompileStatic\n"+
                     ...
                     "    } \n  } \n"+
                     "\n fx0 = new fx();  // the object that performs the mapping computation  \n"+
...
// use user settings for edit frames to adjust location and size
 ...
 fileMenu.add(exitJMenuItem); 
    ...
    public gLabEditor(String selectedValue) {
        ...
        gLabEdit.DefaultImports.scanDefaultImports("DefaultImports.txt");
 }
